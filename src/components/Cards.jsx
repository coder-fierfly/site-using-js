                            ...
                            <>
                                ...
                                <div className="text_group">
                                    {(selectedIndex === item.event_id && item.name !== '') ?
                                        (<p className="item_1 item_bold">{MatchMapping[item.state].description}</p>) : (item.name === '' ? (null) : (<p className="item_1">{MatchMapping[item.state].description}</p>))}

                                    {(selectedIndex === item.event_id && item.name !== '') ?
                                        (<div className="container element_top"><p><span className="appellation_element">Имя устройства:&nbsp;</span>
                                    ...
                                    {(selectedIndex === item.event_id && item.alert_template !== '') ? (<div className="item_5"><p><span className="appellation_element">Шаблон сигнала:&nbsp;</span><span className="item_element">{item.alert_template}</span></p></div>) : (null)}

                                    {(selectedIndex === item.event_id && item.value.emp !== '') ? (<div className="item_5"><p><span className="appellation_element">Значение:&nbsp;</span><span className="item_element">{item.value} {item.units}</span></p></div>) : (null)}
